...
args.sub_dir = (
    ...
    + str(args.mutation)
    + "_r"
    + str(args.recombination)
    + "_kw"
    + str(args.use_max_kw)
    + "_lm"
    + str(args.loss_margin)
)
args.result_dir = os.path.join(args.result_dir, args.sub_dir)
os.makedirs(args.result_dir, exist_ok=True)


def plot_g_converters(
    g_converter_baseline: Type[BaseConductanceConverter],
    ...
    f_lst = (
        [1.0]
        if isinstance(g_converter_baseline, SinglePairConductanceConverter)
        else g_converter_baseline.f_lst
    )
    ...
    w_unitless = np.linspace(
        -max_abs_w_unitless, max_abs_w_unitless, len(g_converter_optimized.g_lst[0])
    )
    w_uS = np.zeros_like(w_unitless)
    ...
    plt.ylabel(r"$Conductance \ [\mu S]$")
    title_str = (
        type(g_converter_optimized).__name__
        + " ("
        + ", ".join([r"$f_{%d}=%0.1f$" % (i, f) for i, f in enumerate(f_lst)])
        + ")"
    )
...
def plot_contour(
    ...
    effective_vals_lst = [w.flatten() for w in effective_vals_lst]

    for i, (label, effective_vals) in enumerate(zip(time_dict.keys(), effective_vals_lst)):
        ...
        density, xbins, ybins = np.histogram2d(
            ...
            bins=200,
            range=[
                [np.amin(ideal_vals), np.amax(ideal_vals)],
                [np.amin(effective_vals_lst[-1]), np.amax(effective_vals_lst[-1])],
            ],
            density=True,
        )

        ratio = (float(i) + 1.0) / len(effective_vals_lst)
        rbg = (
            np.clip(rmin + (rmax - rmin) * ratio, rmin, rmax),
...
def plot_errors(
    ...
    effective_weights_time_lst = [w.flatten() for w in effective_weights_time_lst]

    for i, (label, effective_weights) in enumerate(
        zip(time_dict.keys(), effective_weights_time_lst)
    ):

        ratio = (float(i) + 1.0) / len(effective_weights_time_lst)
        rbg = (
            np.clip(rmin + (rmax - rmin) * ratio, rmin, rmax),
            np.clip(bmin + (bmax - bmin) * ratio, bmin, bmax),
            np.clip(gmin + (gmax - gmin) * ratio, gmin, gmax),
        ...
        plt.hist(
            effective_weights - ideal_weights,
            density=True,
            bins=100,
            color=rbg,
...
def plot_weight_comparison(d: Dict, suffix: str = ""):
    ...
    """
    print(
        "Plotting weight dispersion comparison over time: %s"
    ...
    title_str = "%s (Loss = %0.6f)" % (
        d["baseline_title"],
        np.round(d["baseline_loss"], decimals=6),
    )
    ...
    title_str = "%s (Loss = %0.6f)" % (
        d["optimized_title"],
        np.round(d["optimized_loss"], decimals=6),
    )
    plt.xlim(-xmax, xmax)
    plt.ylim(-ymax, ymax)
    plt.plot(np.linspace(-xymax, xymax, 2), np.linspace(-xymax, xymax, 2), "--", c="gray")
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(args.result_dir, d["baseline_title"] + "_w_comparison_" + suffix + ".png")
    )
    plt.close()
    ...
    plt.savefig(
        os.path.join(
            args.result_dir, d["baseline_title"] + "_w_error_comparison_" + suffix + ".png"
        )
    )
    plt.close()
...
def plot_z_comparison(d: Dict, suffix: str = ""):
    ...
    """
    print(
        "Plotting z dispersion comparison over time: %s"
        % os.path.join(args.result_dir, d["baseline_title"] + "_comparison.png")
    )

    xmax = np.amax(np.abs(np.asarray(d["z_ideal"].detach().cpu().numpy())))
    ymax = np.amax(np.abs(np.asarray(d["z_baseline_time_lst"] + d["z_optimized_time_lst"])))
    xymax = max([xmax, ymax])
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(args.result_dir, d["baseline_title"] + "_z_comparison_" + suffix + ".png")
    )
    plt.close()
    ...
    plt.savefig(
        os.path.join(
            args.result_dir, d["baseline_title"] + "_z_error_comparison_" + suffix + ".png"
        )
    )
    plt.close()
...
def plot_accuracy_comparison(acc_dict: Dict, suffix: str = ""):
    ...
    print(
        "Plotting accuracy comparison over time: %s"
...
def plot_device_models(rpu_config, filename):
    ...
    g_lin = torch.linspace(
        rpu_config.noise_model.g_converter.g_min, rpu_config.noise_model.g_converter.g_max, 1000
    )
    ...
    g_read_noise = rpu_config.noise_model.apply_drift_noise_to_conductance(
        gs, torch.zeros_like(gs), 1.0
    )
    ...
    plt.fill_between(g_lin, g_nu.mean(0) - g_nu.std(0), g_nu.mean(0) + g_nu.std(0), alpha=0.2)
    plt.xlabel(r"$Conductance \ [\mu S]$")
...
def create_datasets():
    ...
    eval_data = squad.map(preprocess_validation, batched=True, remove_columns=squad.column_names)
...
def do_inference(model, trainer, squad, eval_data, time_dict):
    ...
    def predict():
        ...
        predictions = postprocess_predictions(squad, eval_data, raw_predictions.predictions)
    ...
    results_dict = {"f1_lst": f1_lst, "exact_match_lst": exact_match_lst, "time_dict": time_dict}
    return results_dict
...
def plot_weight_distribution(weights, suffix=""):
    ...
    plt.xlabel("Weights [1]")
    plt.ylabel("Density [1]")
    plt.title("%0.1f million weights" % (1.0e-6 * weights.numel()))
...
def main():
    ...
    g_converter_baseline = DualPairConductanceConverter(f_lst=[1.0, 1.0], g_min=g_min, g_max=g_max)
    ...
    plot_device_models(rpu_config_baseline, os.path.join(args.result_dir, "device_models.png"))
    ...
    if args.optimize:
        ...
        save_object(
            rpu_config_optimized,
            os.path.join(args.result_dir, "rpu_config_optimized_" + suffix + ".pkl"),
        )
        save_object(
            wpo, os.path.join(args.result_dir, "WeightProgrammingOptimizer_" + suffix + ".pkl")
        )
    else:
    ...
    if args.inference or args.optimize:
        ...
        fc_baseline = (
            AnalogLinear(IN_FEATURES, OUT_FEATURES, bias=False, rpu_config=rpu_config_baseline)
            .to(DEVICE)
            .eval()
        )

        fc_optimized = (
            AnalogLinear(IN_FEATURES, OUT_FEATURES, bias=False, rpu_config=rpu_config_optimized)
            .to(DEVICE)
            .eval()
        )

        weights_test = downsample_weight_distribution(
            weights_ideal, torch.Size([IN_FEATURES, OUT_FEATURES])
        ).to(DEVICE)
        ...
        # create weight info
        weight_dict = {
            "ideal_weights": weights_test,
            "time_dict": time_dict,
            "effective_weights_baseline_time_lst": effective_weights_baseline_time_lst,
            "effective_weights_optimized_time_lst": effective_weights_optimized_time_lst,
            "baseline_title": type(g_converter_baseline).__name__,
            "optimized_title": type(g_converter_optimized).__name__,
            "baseline_loss": loss_weights(
                fc_baseline.to("cpu"),
                time_steps,
                weights_test.to("cpu"),
                max_abs_weight.to("cpu"),
                0,
                0,
                get_baseline=True,
            ),
            "optimized_loss": loss_weights(
                fc_optimized.to("cpu"),
                time_steps,
                weights_test.to("cpu"),
                max_abs_weight.to("cpu"),
        ...
        z_dict = {
            ...
            "baseline_loss": np.mean(
                np.stack(
                    [(z - z_ideal.detach().cpu().numpy()) ** 2 for z in z_baseline_time_lst], axis=1
                )
            ),
            "optimized_loss": np.mean(
                np.stack(
                    [(z - z_ideal.detach().cpu().numpy()) ** 2 for z in z_optimized_time_lst],
                    axis=1,
                )
            ),
        }
        ...
        accuracy_baseline = do_inference(
            model_baseline, trainer_baseline, squad, eval_data, time_dict
        )
        ...
        accuracy_optimized = do_inference(
            model_optimized, trainer_optimized, squad, eval_data, time_dict
        )
        ...
        save_object(
            accuracy_dict, os.path.join(args.result_dir, "accuracy_dict_" + suffix + ".pkl")
        )
        ...
        try:
            ...
            z_dict = load_object(os.path.join(args.result_dir, "z_dict_" + suffix + ".pkl"))
