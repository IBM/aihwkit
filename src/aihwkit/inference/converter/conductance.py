...
class NPairConductanceConverter(BaseConductanceConverter):
    ...
    def convert_to_conductances(self, weights: Tensor) -> Tuple[List[Tensor], Dict]:
        ...
        for f_factor in self.f_lst:
            conductances.append(
                (
                    (weights_us.clamp(min=0.0) - lower_bound_us) / f_factor + self.g_min
                ).clamp(min=self.g_min, max=self.g_max)
            )  # g_plus
            conductances.append(
                (
                    ((-weights_us).clamp(min=0.0) - lower_bound_us) / f_factor
                    + self.g_min
                ).clamp(min=self.g_min, max=self.g_max)
            )  # g_minus
...
class CustomPairConductanceConverter(BaseConductanceConverter):
    ...
    def __init__(
        ...
        if 2 * len(f_lst) != len(g_lst):
            raise ValueError(
                "must have one value in f_lst for every pair of values in g_lst"
            )
    ...
    def invertibility_test(self) -> None:
        ...
        if not allclose(test_weights, return_weights, atol=0.0001):
            raise ArithmeticError(
                "CustomPairConductanceConverter is not an invertible function"
            )
    ...
    def convert_to_conductances(self, weights: Tensor) -> Tuple[List[Tensor], Dict]:
        ...
        for f_factor, (gp_lst, gm_lst) in zip(
            self.f_lst, zip(self.g_lst[::2], self.g_lst[1::2])
        ):
        ...
        w_lst = (
            linspace(-max_weight_us, max_weight_us, len(self.g_lst[0])) / scale_ratio
        ).tolist()
        ...
        for f_factor, (gp_lst, gm_lst) in zip(
            ...
            conductances.append(
                from_numpy(interp(weights.cpu().numpy(), w_lst, gp_lst)).type_as(
                    weights
                )
            )
            conductances.append(
                from_numpy(interp(weights.cpu().numpy(), w_lst, gm_lst)).type_as(
                    weights
                )
            )
    ...
    def convert_back_to_weights(
        ...
        if 2 * len(params["f_lst"]) != len(conductances):
            raise ValueError(
                "must have one value in f_lst for every pair of conductances"
            )
