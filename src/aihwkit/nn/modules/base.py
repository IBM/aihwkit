...
class AnalogLayerBase:
    ...
    def load_state_dict(
        ...
        # For the checkpoint saved by aihwkit before version, 0.9.2, the parameters with class `AnalogContext`
        #    are saved as empty size tensors since `AnalogContext`, which is a derived class of `torch.nn.Parameter`,
        ...
        if len(keys_to_delete) > 0:
            ...
            for key in keys_to_delete:
                del state_dict[key]
            warnings.warn(
                "Some parameters in the loaded checkpoint has empty size (param.size() == torch.Size([]))."
                "It could happens because of the loaded checkpoint is generated by an older version of aihwkit."
                "The parameter is skipped for compatibility reasons and the loading mode is set to non-strict."
