...
class AnalogBertEmbeddings(Module):
    ...
    def __init__(self, config):
        ...
        if version.parse(torch.__version__) > version.parse("1.6.0"):
            self.register_buffer(
                "token_type_ids",
                zeros(self.position_ids.size(), dtype=long),
                persistent=False,
            )
...
class AnalogBertModel(AnalogSequential):
    ...
    def __init__(self,
        ...
        self.embeddings = AnalogBertEmbeddings(config)
        self.encoder = AnalogBertEncoder(config, rpu_config, realistic_read_write=realistic_read_write)
