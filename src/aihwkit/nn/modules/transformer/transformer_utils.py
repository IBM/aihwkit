...
def prune_linear_layer(layer: AnalogLinear, index: LongTensor, dim: int = 0) -> AnalogLinear:
    ...
    Args:
        layer (`AnalogLinear`): The layer to prune.
        index (`LongTensor`): The indices to keep in the layer.
        dim (`int`, *optional*, defaults to 0): The dimension on which to keep the indices.

    Returns:
        `AnalogLinear`: The pruned layer as a new layer with `requires_grad=True`.
    """
    ...
    new_layer = AnalogLinear(new_size[1], new_size[0], bias=layer.bias is not None).to(layer.weight.device)
...
def apply_chunking_to_forward(
    forward_fn: Callable[..., Tensor], chunk_size: int, chunk_dim: int, *input_tensors
) -> Tensor:
    """Chunk the `input_tensors` into smaller input
    tensor parts of size `chunk_size` over the dimension
    `chunk_dim`. It then applies a layer `forward_fn` to each chunk independently to save memory.
    If the `forward_fn` is independent across the `chunk_dim` this
    function will yield the same result as directly
    applying `forward_fn` to `input_tensors`.
    Args:
        forward_fn (`Callable[..., Tensor]`):
            The forward function of the model.
        chunk_size (`int`):
            The chunk size of a chunked tensor:
    ...
    Returns:
        `torch.Tensor`: A tensor with the same shape as the
        `forward_fn` would have given if applied`.
    ...
    if chunk_size > 0:
        ...
        input_tensors_chunks = [tuple(input_tensor.chunk(num_chunks, dim=chunk_dim) 
                                    for input_tensor in input_tensors)]
