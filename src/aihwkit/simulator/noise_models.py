...
from torch import Tensor, randn, mul, log, clamp
...
class BaseNoiseModel:
    ...
    def apply_noise(self, weights: Tensor, t_inference: float) -> Tensor:
        ...
        for g_target in target_conductances:
            g_prog = self.apply_programming_noise(g_target)
            if t_inference > 0:
                nu_drift = self.generate_drift_coefficients(g_target)
                noisy_conductances.append(self.apply_drift_noise(g_prog, nu_drift, t_inference))
    ...
    @no_grad()
    def apply_programming_noise(self, g_target: Tensor) -> Tensor:
...
class PCMLikeNoiseModel(BaseNoiseModel):
    ...
    def __init__(self,
                 ...
                 t_0: float = 20.0):
        g_converter = g_converter or SinglePairConductanceConverter(g_max=g_max)
    ...
    def apply_programming_noise(self, g_target: Tensor) -> Tensor:
        ...
        g_prog = g_target + mul(sig_prog, randn(*g_target.size()))
    ...
    @no_grad()
    def generate_drift_coefficients(self, g_target: Tensor) -> Tensor:
        """Generates drift coefficients ``nu`` based on PCM measurements."""
        ...
        nu_drift = torch_abs(mu_drift + sig_drift * randn(*g_relative.size())).clamp(min=0.0)
    ...
    def apply_drift_noise(
        ...
        if t > 0:
            q_s = (0.0088 / ((torch_abs(g_prog) / self.g_max) ** 0.65).clamp(
                min=1e-3)).clamp(max=0.2)
            sig_noise = q_s * sqrt(numpy_log((t + self.t_read) / (2 * self.t_read)))
            g_final = g_drift + torch_abs(g_drift) * sig_noise * randn(*g_prog.size())
        else:
            g_final = g_prog
