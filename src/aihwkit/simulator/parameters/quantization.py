...
class BaseQuantConfig(_PrintableMixin):
    ...
    def range_estimator_params(self, new_params):
        ...
        if expected_type is None and new_params is not None:
            raise TypeError(
                f"Range estimator 'allminmax' does not accept a range_estimator params object"
            )
        elif not isinstance(new_params, expected_type):
            raise TypeError(
                f"Expected parameters of type {expected_type.__name__}, got {type(new_params).__name__}"
            )
...
class ActivationQuantConfig(BaseQuantConfig):
    """The quantization config that should be used for activation quantization. This class wraps the
    `BaseQuantConfig` class, by initializing the range_estimator object in the `running_minmax` mode.
    """
    ...
    def __init__(
        ...
        range_estimator : RangeEstimators, optional
            The type of range estimator to use for PTQ or QAT with range estimation, by default RangeEstimators.running_minmax
        range_estimator_params : Optional[Any], optional
            The parameters to configure the `range_estimator` object. Its type must be in accordance with the
            selected range_estimator (see `BaseQuantConfig` for details_. If None is given, the default params
            object, initialized in the `BaseQuantConfig` post_init function will be retained, by default None
        """
...
class WeightQuantConfig(BaseQuantConfig):
    ...
    `BaseQuantConfig` class, by initializing the range_estimator object in the `current_minmax` mode,
    and adding the `per_channel` option.
    """
    ...
    def __init__(
        ...
        range_estimator : RangeEstimators, optional
            The type of range estimator to use for PTQ or QAT with range estimation, by default RangeEstimators.current_minmax
        range_estimator_params : Optional[Any], optional
            The parameters to configure the `range_estimator` object. Its type must be in accordance with the
            selected range_estimator (see `BaseQuantConfig` for details). If None is given, the default params
            object, initialized in the `BaseQuantConfig` post_init function will be retained, by default None
        """
        super().__init__(n_bits=n_bits, symmetric=symmetric, _range_estimator=range_estimator)
        if range_estimator_params is not None:
...
class QuantizationMap(_PrintableMixin):
    ...
    the capability to define specific quantization options per-layer-instance for maximum flexibility but
    also per-module-type to reduce the definition code. See below for the available options and
    how to use each option.
    """

    default_qconfig: QuantizationConfig = field(default_factory=QuantizationConfig)
    """This is a utility field and it is NOT used during the `convert_to_quantized` call. It exists to
    simplify development code in the case where most layers use the same quantization configuration, so 
    that the user defines it here and then shares it when he defines the `instance_qconfig_map`
    and `module_qconfig_map` fields (see `append_default_conversions` function for such a use).
    """
    ...
    is a dictionary where the keys are a type of a Module (e.g., nn.Linear) and the value is an instance
    of `QuantizedModuleConfig`, which includes a quantized Module to replace the original one
    (e.g., QuantLinear) and a `QuantizationConfig` instance with the parameters for the new quantized layer.

    NOTE: This is the lowest priority for the conversions. If an instance of a module is defined in the
    `instance_qconfig_map`, the conversion defined here will be ignored, and the conversion defined in the
    `instance_qconfig_map` will happen. The same applies if an instance of a module is included in the 
    ...
    """This field defines a map of how to convert specific instances of modules to quantized quanterparts. It
    is a dictionary where the keys are the string identifier of a layer, as it appears in the state dict of 
    the model (e.g., \'block1.linear1\') and the value is an instance of `QuantizedModuleConfig`, which includes a quantized Module to replace the original one
    (e.g., QuantLinear) and a `QuantizationConfig` instance with the parameters for the new quantized layer.
    

    NOTE: This takes priority over a possible conversion defined in the `module_qconfig_map` for the type
    of the layer identified by the string identifier (e.g. for QuantLinear layers). If a layer is both included
    in this field and in the `excluded_modules` list (by user error), the latter has priority.
    ...
    """This field defines a map of the modules that should be wrapped in the `QuantizedInputModule` and
    the parameters of how to quantize the input activations. Since the convention used in this database
    is that each quantized layer quantizes its output activations, there are cases that a module could
    receive unquantized data as inputs (for example if it's the first layer, or if it follows a 
    functional call that cannot be quantized with the scheme defined here). For these reasons, an arbitrary
    Module could be wrapped in the `QuantizedInputModule` class to allow for full customizability.
    The modules here are defined based on the string identifier, as appears in the state dict of the model,
    while the value of the dict is an `ActivationQuantConfig` object.

    NOTE: This conversion follows any quantization conversion defined in the `instance_qconfig_map` and 
    `module_qconfig_map`. That means that if a quantization for a module is defined in one of the other
    maps and here, the quantized module will be properly wrapped in the `QuantizedInputModule`.

    Examples
    --------
    >>> # Quantize the inputs of the first layer of a network
    >>> quantization_map.input_activation_qconfig_map[\'firstblock.firstlayer\'] = ActivationQuantConfig(
    >>>     n_bits=8, symmetric=False
    >>> ) 
    """

    excluded_modules: List[str] = field(default_factory=lambda: [])
    """This field is a list of modules, identified by their state dict string, to be excluded from ANY
    conversion that is defined for their type or for their instance. This takes precedence over all
