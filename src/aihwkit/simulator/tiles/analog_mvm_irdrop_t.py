...
class AnalogMVMIRDropT(AnalogMVM):
    ...
    def _matmul_irdrop(
        ...
        ## Activation-Dependent PCM Read-Noise Model
        # More details can be found in publication: https://doi.org/10.48550/arXiv.2506.00004
        if io_pars.apply_xdep_pcm_read_noise: 
            ...
            # sigma noise slope coefficients
            SNSC = [0.00021926, -0.00187352, 0.00655714, -0.0146159, 0.02578764]
            # signma noise offest coefficients
            SNOC = [1.11906167e-09, -9.08576764e-09, 3.07015063e-08, -6.77079241e-08, 1.17763144e-07]
            
            # Convert activations/inputs into integer ns units
            t_integration = torch_mean(torch_abs(input_) / (2*res))
            x = torch_log10(t_integration) if t_integration > 0. else torch.tensor(0.,
                                                                                   dtype=t_integration.dtype,
                                                                                   device=t_integration.device)
            ...
            sig_noise_slope  =   SNSC[0]*(x**4) + SNSC[1]*(x**3) + SNSC[2]*(x**2) + SNSC[3]*(x**1) + SNSC[4]
            sig_noise_offset = ( SNOC[0]*(x**4) + SNOC[1]*(x**3) + SNOC[2]*(x**2) + SNOC[3]*(x**1) + SNOC[4] ) * 1e6 #[uS] 
        ...
        if io_pars.adc_quantization:
            adcTicks_even = trunc((mvm_even_col_down_adc / 100) * io_pars.adc_frequency)
            mvm_even_col_down_adc = adcTicks_even * 100 / io_pars.adc_frequency   
        ...
        if io_pars.adc_quantization:
            adcTicks_odd = trunc((mvm_odd_col_up_adc / 100) * io_pars.adc_frequency)
            mvm_odd_col_up_adc = adcTicks_odd * 100 / io_pars.adc_frequency
    ...
    def _compute_analog_mv(  # type: ignore
        ...
        **fwd_pars: Any,
        ...
        if io_pars.mv_type == AnalogMVType.ONE_PASS:
            ...
            finalized_outputs = ( finalized_outputs_lsb + (2**io_pars.split_mode_bit_shift) * finalized_outputs_msb )
