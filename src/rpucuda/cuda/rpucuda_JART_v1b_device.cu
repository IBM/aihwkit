...
template <typename T> struct UpdateFunctorJARTv1bMult {
...
template <typename T> struct UpdateFunctorJARTv1bAdd {
...
pwukpvec_t<T> JARTv1bRPUDeviceCuda<T>::getUpdateKernels(
  ...
  if (par.ls_mult_noise) {
    v.push_back(
        RPU::make_unique<PWUKernelParameterSingleFunctor<T, UpdateFunctorJARTv1bMult<T>, 1>>
            ARGS);
    v.push_back(
        RPU::make_unique<PWUKernelParameterBatchFunctor<T, UpdateFunctorJARTv1bMult<T>, 1>>
            ARGS);
    v.push_back(
        RPU::make_unique<PWUKernelParameterBatchSharedFunctor<T, UpdateFunctorJARTv1bMult<T>, 1>>
            ARGS);

  } else {

    v.push_back(
        RPU::make_unique<PWUKernelParameterSingleFunctor<T, UpdateFunctorJARTv1bAdd<T>, 1>>
            ARGS);
    v.push_back(
        RPU::make_unique<PWUKernelParameterBatchFunctor<T, UpdateFunctorJARTv1bAdd<T>, 1>> ARGS);
    v.push_back(
        RPU::make_unique<PWUKernelParameterBatchSharedFunctor<T, UpdateFunctorJARTv1bAdd<T>, 1>>
            ARGS);
  }
  return v;
}
