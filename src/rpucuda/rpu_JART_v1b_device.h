...
// #define PHYSICAL_PARAMETER_kb 1.3807e-23								//
// Boltzman's constant  [VAs/K]
...
BUILD_PULSED_DEVICE_META_PARAMETER(
    ...
    T Ndiscmin = (T)0.008; // from [0.0001:100];			// minimum oxygen vacancy concentration in
                           // the disc [10^26/m^3]
    ...
    T a = (T)0.25e-9; // from [0.1e-9:1e-9];					// ion hopping
                      // distance [m]
    T ny0 =
        (T)2e13;         // from [1e10:1e14];					// attemp frequenzy [Hz]
    T dWa = (T)1.35;     // from [0.8:1.5];					// activation energy [eV]
    T Rth0 = (T)15.72e6; // from [1e6:20e6];					// thermal resistance of the
                         // Hafnium Oxide [K/W]
    T rdisc = (T)45e-9; // from [5e-9:100e-9];				// radius of the filament
                        // area [m]
    T lcell = (T)3 * 1e-9; // from [2:5];							// length of disc and plug
                           // region [m]
    T ldisc =
        (T)0.4 *
        1e-9; // from [0.1:5]; 					// length of the disc region [m]
    T Rtheff_scaling =
    ...
    T read_voltage = (T)0.2; T pulse_voltage_SET = (T)-0.342; T pulse_voltage_RESET = (T)0.7065;
    T pulse_length = (T)1e-6; T base_time_step = (T)1e-8; T Ndisc_min_bound = (T)0.06;
    T Ndisc_max_bound = (T)1.9897452127440086504; T w_min = (T)-0.6; T w_min_dtod = (T)0.3;
    T w_max = (T)0.6; T w_max_dtod = (T)0.3; T Ndiscmax_dtod = (T)0.0; //
    ...
    T current_min = (T)(-g0 * (exp(-g1 * read_voltage) - 1)) /
                    (pow(
                        (1 + (h0 + h1 * read_voltage + h2 * exp(-h3 * read_voltage)) *
                                 pow((Ndisc_min_bound / Ndiscmin), (-j_0))),
                        (1 / k0)));
    T current_max = (T)(-g0 * (exp(-g1 * read_voltage) - 1)) /
                    (pow(
                        (1 + (h0 + h1 * read_voltage + h2 * exp(-h3 * read_voltage)) *
    ...
    T Ninit =
        pow(((pow(((-g0 * (exp(-g1 * read_voltage) - 1)) /
                   (((0 - w_min) / (w_max - w_min)) * (current_max - current_min) + current_min)),
                  k0) -
              1) /
             (h0 + h1 * read_voltage + h2 * exp(-h3 * read_voltage))),
            (1 / -j_0)) *
        Ndiscmin; // from [0.0001:1000];				// initial oxygen vacancy concentration in
                  // the disc [10^26/m^3]

    T alpha_SET = (T)((alpha1 + alpha0) / (1 + exp(-(pulse_voltage_SET + alpha2) / alpha3))) -
                  alpha0;
    ...
    T gamma_coefficient = (T)(PHYSICAL_PARAMETER_zvo * a) / (dWa * M_PI); T a_ny0 = (T)a * ny0;
    ...
    ss << "\t alpha0:\t\t" << alpha0 << std::endl; ss << "\t alpha1:\t\t" << alpha1 << std::endl;
...
template <typename T> class JARTv1bRPUDevice : public PulsedRPUDevice<T> {
  ...
  BUILD_PULSED_DEVICE_CONSTRUCTORS(
      ...
      swap(
          a.device_specific_Ndiscmax_ctoc_upper_bound, b.device_specific_Ndiscmax_ctoc_upper_bound);
      swap(
          a.device_specific_Ndiscmin_ctoc_upper_bound, b.device_specific_Ndiscmin_ctoc_upper_bound);
      ...
      swap(
          a.device_specific_Ndiscmax_ctoc_lower_bound, b.device_specific_Ndiscmax_ctoc_lower_bound);
      swap(
          a.device_specific_Ndiscmin_ctoc_lower_bound, b.device_specific_Ndiscmin_ctoc_lower_bound);
